...
generalEquations = [scheduleEquations;  energyConversionEquations;  capacityEquations; reformulationEquations;]; 
generalConstraints = [generalEquations <= 0];
energyConversionEquations1 = replace(energyConversionEquations,[ei,gi],[ei_hat,gi_hat]);
SPequations = [scheduleEquations;  energyConversionEquations1;  capacityEquations;reformulationEquations;];
...
elseif diagnostics.problem == 0 %feasible
    ...
    elseif diagnostics.problem ~= 0 % means we need shifting or curtailing
            ...
            for r2 = 1:size(sw_relaxed,2)
                for r3 = 1:size(sw_relaxed,3)
                    if value(sw_relaxed(r1,r2,r3)) ~= fix(value(sw_relaxed(r1,r2,r3)))
                        ...
                        % L&P cut generation
                        [alpha,beta0] = LPcutGeneration(ei_hat,gi_hat,x_hat_relaxed,-generalmodel.A,-generalmodel.rhs,r);
                        % cut
                        addLPcutEquation_general = [
                                    addLPcutEquation_general;
                                    -alpha'*recover(generalrecoverymodel.used_variables) + beta0;
                                    ];
                    end
                end
            end
        ...
        addLPcutEquation = replace(addLPcutEquation_general,[ei,gi],[ei_hat,gi_hat]);
...
for l = 1:3 % 3 energies
    for ift = 1:NK.DR % from time
        curtailmentCost = curtailmentCost + lc(ift,l) * CDF*1000000;
        ft = iK.DR(ift);
