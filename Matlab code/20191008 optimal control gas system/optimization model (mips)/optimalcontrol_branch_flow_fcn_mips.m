function [h,dh] = optimalcontrol_branch_flow_fcn_mips(x, mpc, Yf, Yt, il, mpopt,NK)
nl = size(mpc.branch,1); nb = size(mpc.bus,1);
[Va, Vm] = deal(x{:});
[nVa,nVm] = deal(size(Va,1),size(Vm,1));
cumnvar = cumsum([nVa,nVm]);
nvar = nVa+nVm;
[Va0, Vm0] = deal(reshape(Va,[],NK),reshape(Vm,[],NK));%ÁÐÊÇk
h = ones(NK*2*nl,1); dh = ones(NK*2*nl,nvar);[h0,dh0] = deal(cell(NK,1));
for k = 1:NK
    Va = Va0(:,k); Vm = Vm0(:,k);
    [h0{k},dh0{k}] = opf_branch_flow_fcn(Va,Vm, mpc, Yf, Yt, il, mpopt);
    h((k-1)*2*nl+1:k*2*nl) = h0{k};  
end
for i = 1:nvar
    ...
    dh((k-1)*2*nl+1:k*2*nl,i) = dh0{k}(:,itype);
end
...
function [h, dh] = opf_branch_flow_fcn(Va,Vm, mpc, Yf, Yt, il, mpopt)
%%----- initialize -----
...
%                     conjYfV = conj(Yf * V); conjYtV = conj(Yt * V);
%                     for i = 1:size(il,1)
%                         Sf(i) = V(branch(i, F_BUS))*conjYfV(i);
%                         St(i) = V(branch(i, T_BUS))*conjYtV(i);
%                     end
%                     Sf = Sf';St = St';
