function [gasLC,GFUcapacity] = calculateGasLCv2(transientResult,steadyResult,newmpc,nt)
% the difference to version 1, the distribution of GFU 
%% initialization
NK = sum(nt);
...
ngfu = size(newmpc.gfuIndex,2);
gfuUnitIndex = newmpc.gfuIndex;
GFUcapacity = zeros(NK,ngfu);
GFUelectricityBus = newmpc.gen(gfuUnitIndex,1);
GFUbus = zeros(size(GFUelectricityBus));
for i = 1:size(GFUelectricityBus,1)   
    GFUbus(i) = newmpc.GEcon(find(newmpc.GEcon(:,2)==GFUelectricityBus(i)),1); %GFU gas bus,sorted by gen order
end
gfuEfficiency = 200;
...
for k = 1:NK
    j = max(find(cumnt<k));
    GFUgasRequirement = steadyResult{j}.gen(newmpc.gfuIndex,9) / gfuEfficiency;% ATTENTION!! this is actually the "capacity"
...
%     end
   ...
   for i = 1:nGb
       if ismember(i,gasLoadBus) && ismember(i,GFUbus)
           gfuIndexIndex = find(GFUbus==i); % the index of gfuIndex,may be a matrix
           ...
           if Qinjection(i)>=steadyResult{j}.Gbus(i,3)+sum(GFUgasRequirement(gfuIndexIndex))
               gasLC(k,gasLoadBusIndex) = 0;
               GFUcapacity(k,gfuIndexIndex) = steadyResult{j}.gen(GFUbus(gfuIndexIndex),2);
           end
       end
       %
       if ismember(i,gasLoadBus) && ~ismember(i,GFUbus)
