...
% initialQ = sdpvar(NK,sum(nx+1)); %每个场景、每个调度时段相同
% PGsmaxScenario = sdpvar(NS,NK,nGs);
% PgmaxScenario = sdpvar(NS,NK,ngen); % 每个场景相同，2个调度时段不同，其中第NS个场景为元件状态不变化的
...
%% initial values IEGS 每个场景下的初值是相同的
for k = 1:NK
    Pg0(k,:) = dayahead_IEGSresult_basicLoad{k}.gen(:, PG) / baseMVA; % 
    Va0(k,:)  = dayahead_IEGSresult_basicLoad{k}.bus(:, VA) * (pi/180);% 弧度
...
assign(LCg,LCg0);
assign(ei,ei0); assign(gi,gi0); % 注意天然气单位的折算
...
initialQ = zeros(NK,sum(nx+1)); %每个场景、每个调度时段相同 (Mm3/day)
