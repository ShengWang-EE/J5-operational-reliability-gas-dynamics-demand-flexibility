function [MP_IEGSresult, solverTime, exitflag,constraints] = MP_IEGSdispatch(addCutsCoefficient,exitflagSP,SP_EHresult,...
...
% dayahead_IEGSresult_basicLoad = dayahead_IEGSresult;
% for h = 1:NH
%     for i = 1:nEH
%         EHgbus = mpc0.EHlocation(i,1); EHebus = mpc0.EHlocation(i,2); 
%         dayahead_IEGSresult_basicLoad{h}.bus(EHebus,3:4) = dayahead_IEGSresult{h}.bus(EHebus,3:4) * ...
%             (1-dayaheadEHschedule{i}(h,1)/dayahead_IEGSresult{h}.bus(EHebus,3));
%         dayahead_IEGSresult_basicLoad{h}.Gbus(EHgbus,3) = dayahead_IEGSresult{h}.Gbus(EHgbus,3) * ...
%             (1-dayaheadEHschedule{i}(h,2)/200)/dayahead_IEGSresult{h}.Gbus(EHgbus,3);
%     end
% end

%%
slack1 = sdpvar(size(optimalcontrol_dynamicGasflow_fcn_motion(Prs,Gf,avrgQP,...
    dayahead_IEGSresult_basicLoad,gtd,initialCondition,para,vv,dx,nx,dt,NK.studyPeriod),1),1);
slack2 = sdpvar(NK.studyPeriod,sum(nx+1));
...
dynamicGasFlowMotionCons = [optimalcontrol_dynamicGasflow_fcn_motion(Prs,Gf,avrgQP,...
    dayahead_IEGSresult_basicLoad,gtd,initialCondition,para,vv,dx,nx,dt,NK.studyPeriod) <= 10+slack1]:'dynamicGasFlowMotionCons';
% dynamicGasFlowCons = [-slack <= optimalcontrol_dynamicGasflow_fcn_yalmip(Prs,Gf,avrgQP,...
%     dayahead_IEGSresult_basicLoad,gtd,initialCondition,para,vv,dx,nx,dt,NK.studyPeriod) <= slack]:'dynamicGasFlowCons';
...
%% add cut
% % EHcostCons = [EHcost_hat >= 0;]; 
% addCutEquations = [];addCutRHSs = [];
% if ~isempty(SP_EHresult) && ~isempty(addCutsCoefficient)
%     for iter = 2:Niter-1
%         for i = 1:nEH
% %             EHcostCons = [EHcostCons;EHcost_hat >= SP_EHresult{iter,i}.cost_hat];
%             A.ei = addCutsCoefficient{iter,i}.ei.A; A.gi = addCutsCoefficient{iter,i}.gi.A; 
%             rhs = addCutsCoefficient{iter,i}.rhs;            
%             if exitflagSP(iter,i) == 0 % SP infeasible   
%                 addCutEquations = [addCutEquations; A.ei * baseMVA * ei(:,i) + A.gi * 200 * gi(:,i);]; % 注意gi前面的系数需要折算
...
if ~isempty(SP_EHresult) && ~isempty(addCutsCoefficient)
    for iter = 2:Niter-1
        for i = 1:nEH
%             EHcostCons = [EHcostCons;EHcost_hat >= SP_EHresult{iter,i}.cost_hat];
            dual_ei = addCutsCoefficient{iter,i}.dual_ei; dual_gi = addCutsCoefficient{iter,i}.dual_gi;
            ei_hat = addCutsCoefficient{iter,i}.ei_hat;   gi_hat = addCutsCoefficient{iter,i}.gi_hat;
            EHcost_hat_hat = addCutsCoefficient{iter,i}.EHcost_hat;   
            addCutEquations = [addCutEquations; 
                EHcost_hat_hat - dual_ei'*(baseMVA*ei(:,i)-ei_hat) - dual_gi'*(200*gi(:,i)-gi_hat);]; % 注意gi前面的系数需要折算
            if exitflagSP(iter,i) == 0 % SP infeasible   
...
constraintViolation = sum(slack1);


objfcn = IEGScost + sum(EHcost_hat) +  1e6*constraintViolation + giCost;
