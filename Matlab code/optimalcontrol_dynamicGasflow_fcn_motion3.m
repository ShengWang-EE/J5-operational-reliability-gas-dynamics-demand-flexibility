function [f] = optimalcontrol_dynamicGasflow_fcn_motion3(p0,q0,mpc,gtd,initialCondition,initialP,initialQ,para,vv,dx,nx,dt,NK)
...
% %         avrgQP = mean([terminalCondition.Q{i}(1,m-1:m)./terminalCondition.P{i}(1,m-1:m)]);
        for k = 2:NK+1
            ...
            P_0 = initialCondition.P{i}(m-1);P_X0 = initialCondition.P{i}(m); %psia
            Q_0 = initialCondition.Q{i}(m-1);Q_X0 = initialCondition.Q{i}(m);% 本次优化中初始时刻的值（与上下时间步长的无关）
...
%                 f_continuityAndMontion(iMotion{i}(k-1,m-1)) = ...
%                 (1-signGf)/2 * norm([(P_T+P),sqrt(C_q^2*B^2*dl/(F^2*D*A^2))/1e6*(Q_XT+Q_X+Q_T+Q)]) - (P_XT+P_X) +  ...
%                 (1+signGf)/2 * norm([(P_XT+P_X),sqrt(C_q^2*B^2*dl/(F^2*D*A^2))/1e6*(Q_XT+Q_X+Q_T+Q)]) - (P_T+P);
% v3: 自适应线性化 
         f_continuityAndMontion(iMotion{i}(k-1,m-1)) = ...
           (P_XT-P_T+P_X-P)/2/dl - (P_X0-P_0)/dl + (4*C_q^2*B^2/(F^2*D*A^2)) * (Q_X0+Q_0)/(P_X0+P_0) * ...
           ( (Q_XT+Q_X+Q_T+Q)/4 - (Q_X0+Q_0)/2 );
